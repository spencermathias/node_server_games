shared = {
	...
	checkNeighbors: function(oldboard,row,col){
		...
		//console.log('{quinto_shared}', "row", row, "centerRow" , centerRow);
		//console.log('{quinto_shared}', "col", col, "centerCol" , centerCol);
		...
		//console.log('{quinto_shared}', "is connected", isConnected);
		return isConnected;
	},
	...
	boardIsCorrectSize: function(submittedBoardState, boardState){
		...
		if(submittedBoardState.length != boardState.length){
			console.log('{quinto_shared}',"Invalid number of rows!"); 
			correctSize = false;
		} 
		for(var row = 0; row < boardState.length; row++){
			//make sure column lengths match
			if(submittedBoardState[row].length != boardState[row].length){
				console.log('{quinto_shared}',"Invalid number of columns!"); 
				correctSize = false;
			}
		}
		return correctSize;
